...
class IRL_Env4LHF:
    ...
    def reset(self, max_traj_len_batch):
        ...
        # manual initializacion, where initial fixation is provided
            ...
            self.fixations[:, 0, 0] = torch.tensor(self.init_fix[:, 0] * self.pa.patch_num[0], dtype=torch.long)
            self.fixations[:, 0, 1] = torch.tensor(self.init_fix[:, 1] * self.pa.patch_num[1], dtype=torch.long)
            bs = self.action_mask.size(0)
            self.action_mask = self.action_mask.view(bs, self.pa.patch_num[1], -1)

            index = 0
            for action_mask in self.action_mask:
                px, py = int(self.init_fix[index, 0] * self.pa.patch_num[0]), int(self.init_fix[index, 1] * self.pa.patch_num[1])
