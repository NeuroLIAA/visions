...
for cfg_file in cfg_files:
    ...
    target_cell_top_left = np.array(cfg_info['target_center_top_left'][0][0][0])
    target_cell_bot_right = np.array(cfg_info['target_center_bot_right'][0][0][0])
    target_size = np.array(cfg_info['target_size'][0][0][0])
    target_center = np.array(cfg_info['target_center'][0][0][0])

    target_top_left = target_center - target_size/2
    target_bot_right = target_center + target_size/2
    
    
    target_bbox = [int(target_top_left[0]), int(target_top_left[1]), int(target_bot_right[0]), int(target_bot_right[1])]

    number_of_fixations = len(scanpath)
    last_fixation = np.array(scanpath[number_of_fixations - 1])
    
    between_bounds = (target_cell_top_left <= last_fixation) & (target_cell_bot_right <= last_fixation)
    ...
    for i in range(number_of_fixations):
        ...
        scanpath_x.append(int(i_fixation[1]) - 1)
        scanpath_y.append(int(i_fixation[0]) - 1)
