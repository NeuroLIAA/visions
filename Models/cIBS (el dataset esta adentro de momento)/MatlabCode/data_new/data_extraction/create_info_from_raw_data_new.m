...
function create_todo_from_asc (dir_data)
    ...
    for su = 1:length(filenames)
        if ~exist([dir_data 'todo/' upper(filenames{su}(1:end-4)) '_todo.mat'], 'file')
            [todo] = fun_extract_all_eye_data(filenames{su}, [dir_data 'asc/']);
            save([dir_data 'todo/' upper(filenames{su}(1:end-4)) '_todo.mat'], 'todo')
        end
    end
end
...
function create_info_from_todo (dir_data)
    try
        tmp = dir([dir_data 'todo/' '*_todo.mat']); filenames = {tmp.name};
        ...
        for su = 1:length(filenames)
            load([dir_data 'todo/' filenames{su}]);
            load([dir_data 'mat/' upper(filenames{su}(1:end-9)) '.mat']);
            filenames{su}
            msg = [todo.msg];
            ...
            save([dir_data 'info/' upper(filenames{su}(1:end-9)) '_info.mat'], 'info', 'exp');
        end
    catch ME
        disp('Ouch!')
        keyboard;
