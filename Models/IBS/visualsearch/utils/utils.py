...
def save_checkpoint(config, scanpaths, targets_found, trials_properties, time_elapsed, output_path):
    config.pop('image_size'), config.pop('max_saccades'), config.pop('proc_number')
    config.pop('save_probability_maps'), config.pop('human_scanpaths')
    checkpoint = {}
...
def add_scanpath_to_dict(image_name, image_scanpath, target_bbox, target_object, grid, config, dataset_name, dict_):
    ...
    dict_[image_name] = {'subject' : config['name'] + ' Model', 'dataset' : dataset_name, 'image_height' : int(grid.size()[0]), 'image_width' : int(grid.size()[1]), \
